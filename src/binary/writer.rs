...
use crate::{
    bundle::{Key, Resource, ResourceBundle},
    Error,
};

use super::{
    BinHeader, BinIndex, BinReprInfo, BinResBundle, CharsetFamily, FormatVersion, ResDescriptor,
    ResourceReprType, DATA_FORMAT, DATA_VERSION, MAGIC_WORD, PADDED_HEADER_SIZE, REPR_INFO_SIZE,
    SIZE_OF_STRING_CHAR, SYSTEM_ENDIANNESS,
};
...
impl Writer {
    ...
    fn build_string_data(
        ...
        for (string, data) in sorted_strings {
            ...
            match length_words {
                ...
                _ => return Err(Error::new("Unsupported string length.")),
            };
    ...
    fn build_32_bit_resource(
        ...
        match &mut resource.type_data {
            BinResourceTypeData::Array { children } => {
                if children.is_empty() {
                    return Ok(ResDescriptor::new_empty(ResourceReprType::Array));
                }
            ...
            BinResourceTypeData::Table { map, .. } => {
                if map.is_empty() {
                    return Ok(ResDescriptor::new_empty(ResourceReprType::Table));
                }
            ...
            BinResourceTypeData::Binary { binary } => {
                if binary.is_empty() {
                    return Ok(ResDescriptor::new_empty(ResourceReprType::Binary));
                }
            ...
            BinResourceTypeData::IntVector { int_vector } => {
                if int_vector.is_empty() {
                    return Ok(ResDescriptor::new_empty(ResourceReprType::IntVector));
                }

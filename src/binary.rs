...
impl TryFrom<&[u8]> for BinHeader {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        if value.len() < core::mem::size_of::<BinHeader>() {
            return Err(Error::new("Input is too short to contain file header."));
        }
        ...
        match magic {
            ...
            _ => {
                return Err(Error::new(
                    "Invalid resource bundle: magic word does not match.",
                ))
            }
        }
        ...
        if (size as usize) < core::mem::size_of::<BinHeader>() {
            return Err(Error::new(
                "Invalid resource bundle: listed header size is too small.",
            ));
        }
...
impl TryFrom<&[u8]> for BinReprInfo {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        // contents of the repr info, an endian mismatch means that the data
        // we're reading is invalid.
        let (endianness, value) = (Endianness::try_from(value[0])?, &value[1..]);
        if endianness != SYSTEM_ENDIANNESS {
            return Err(Error::new(
                "Reading resource bundles of non-native endianness is not supported.",
            ));
        }
        ...
        // As with endianness, a mismatch in the data format indicates that we
        // are not reading a valid resource bundle.
        let (data_format, value) = ([value[0], value[1], value[2], value[3]], &value[4..]);
        if &data_format != b"ResB" {
            return Err(Error::new(
                "Invalid resource bundle: unexpected data format.",
            ));
        }
...
impl From<BinIndex> for Vec<u8> {
    fn from(value: BinIndex) -> Self {
        ...
        if value.field_count >= 6 {
            bytes.extend_from_slice(&value.bundle_attributes.unwrap().to_ne_bytes());
        }
        ...
        if value.field_count >= 8 {
            bytes.extend_from_slice(&value.pool_checksum.unwrap().to_ne_bytes());
        }
...
impl TryFrom<&[u8]> for BinIndex {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if field_count < 5 {
            ...
            return Err(Error::new("Invalid index field count."));
        }
        ...
        impl TryFrom<$type> for $name {
            ...
            fn try_from(value: $type) -> Result<Self, Self::Error> {
                match value {
                    ...
                    _ => Err(Error::new_from_string(
                        format!("Failed to convert {value} to value of type {}.", stringify!($name))
                    )),
                }
            }
        }
...
impl ResDescriptor {
    ...
    pub const fn new_empty(resource_type: ResourceReprType) -> Self {
    ...
    I: SliceIndex<[u8], Output = [u8]>,
{
    input
        .get(index)
        .ok_or(Error::new("Unexpected end of input."))
}
...
fn read_u16(input: &[u8]) -> Result<(u16, &[u8]), Error> {
    ...
    let bytes = get_subslice(input, ..get_16_bit_offset(1))?
        .try_into()
        .unwrap();
    let value = u16::from_ne_bytes(bytes);

    let rest = input
        .get(get_16_bit_offset(1)..)
        .ok_or(Error::new("Unexpected end of input."))?;
    Ok((value, rest))
}
...
const fn get_16_bit_offset(value: usize) -> usize {
    value * core::mem::size_of::<u16>()
}

#[inline(always)]
/// Gets a byte-aligned offset given a count of 32-bit values.
const fn get_32_bit_offset(value: usize) -> usize {
